...
pipeline {
    ...
    stages {
        ...
        stage ('Run'){
            ...
            steps {
                sh """
                    echo "Creating docker network: '${NAME}-network-${COMMIT}'"
                    docker network create ${NAME}-network-${COMMIT}
                    echo "Starting docker container: ${NAME}-${COMMIT}"
                    docker run -d --name '${NAME}-${COMMIT}' --network ${NAME}-network-${COMMIT} ${REPO}:${COMMIT}
                    echo "Starting docker container: jenkins-integration-tests-${COMMIT}"
        ...
        stage ('Test'){
            ...
            steps {
                sh """
                    docker exec jenkins-integration-tests-${COMMIT} /bin/bash -c 'curl ${NAME}-${COMMIT}:8000'
                """
            }
            post {
                always {
                    sh """
                        echo "Removing docker container: ${NAME}-${COMMIT}"
                        docker rm -vf ${NAME}-${COMMIT}
                        echo "Removing docker container: jenkins-integration-tests-${COMMIT}"
                        docker rm -vf jenkins-integration-tests-${COMMIT}
                        echo "Removing docker network: ${NAME}-network-${COMMIT}"
